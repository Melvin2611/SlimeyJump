...
var sprite: Node = null # Entweder AnimatedSprite2D oder Sprite2D
...
func _physics_process(delta):
	...
	# Sanft drehen
	var diff = goal_rotation - sprite.rotation_degrees
	if abs(diff) > 0.1:
		sprite.rotation_degrees += clamp(diff, -rotation_speed * delta, rotation_speed * delta)
	else:
		sprite.rotation_degrees = goal_rotation
		# Rotation erreicht
		if rotating_to_target:
...
func _on_body_entered(body):
	if body.is_in_group("Player"):
		player = body
		# Suche nach AnimatedSprite2D oder Sprite2D im Player
		sprite = body.get_node_or_null("AnimatedSprite2D")
		if not sprite:
